...
ModifyConstraintStudentsSetNotAvailableTimesForm::ModifyConstraintStudentsSetNotAvailableTimesForm(QWidget* parent, ConstraintStudentsSetNotAvailableTimes* ctr): GenericConstraintForm(parent)
{
	setupUi(this);
...
void ModifyConstraintStudentsSetNotAvailableTimesForm::ok()
{
	...
	if(_ctr->students!=students_name){
		...
		QSet<ConstraintStudentsSetNotAvailableTimes*> cs=TContext::get()->instance.ssnatHash.value(oldName, QSet<ConstraintStudentsSetNotAvailableTimes*>());
		assert(cs.contains(_ctr));
		cs.remove(_ctr);
		TContext::get()->instance.ssnatHash.insert(oldName, cs);

		cs=TContext::get()->instance.ssnatHash.value(newName, QSet<ConstraintStudentsSetNotAvailableTimes*>());
		assert(!cs.contains(_ctr));
		cs.insert(_ctr);
		TContext::get()->instance.ssnatHash.insert(newName, cs);
